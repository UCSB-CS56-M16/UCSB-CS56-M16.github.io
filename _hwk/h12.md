...
This homework concerns <span data-hfdp="3">HFDP Chapter 3</span>, and <span data-hfj="3">HFDP Chapter 13</span>.
...
(10 pts) Write a brief `main` method that writes the string `Hello Disk!` to a file called `myFirstFile.txt`.  You only need the `main` method, not the enclosing class.   Be sure to account for any exceptions that must be declared or caught.
</li>
